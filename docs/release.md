...
This document describes the standard release process for spark-k8s-operator.
...
2. **Docker image** — uses `VERSION` directly:
   `quay.io/zncdatadev/spark-k8s-operator:<version>`
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/spark-k8s-operator:<version>` (OCI registry)
...
```bash
gh api repos/zncdatadev/spark-k8s-operator/git/refs \
  -f ref=refs/heads/release-0.x \
  -f sha=$(gh api repos/zncdatadev/spark-k8s-operator/git/ref/heads/main --jq .object.sha)
```
...
- Docker image is available at
  `quay.io/zncdatadev/spark-k8s-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/spark-k8s-operator:x.y.z-dev`
...
- **Release Image** — Builds and pushes multi-arch Docker image using the
  root Dockerfile to
  `quay.io/zncdatadev/spark-k8s-operator:<version>`, and signs the image with
  Cosign
- **Release Chart** — Publishes the Helm chart to
  `quay.io/kubedoopcharts/spark-k8s-operator:<version>` (OCI registry) and
  updates the
...
spark-k8s-operator follows [Semantic Versioning](https://semver.org/):
